# Описание проекта Open Source TimeCard 200T
## Содержание
...
Оригинальный проект Open Source TimeCard 200T включает открытые IP-ядра от [NetTimeLogic](https://www.nettimelogic.com/) и бесплатные IP-ядра от [Xilinx](https://www.xilinx.com/).
...
- Регистр 0x0200_0000 golden образа показывает: 0x0001_0000
- Регистр 0x0200_0000 обычного образа показывает: 0x0000_0003
...
- Скрипт */[YOUR_PATH]/Implementation/Xilinx/TimeCard_200T/CreateBinaries.tcl* запускает синтез/реализацию проекта TimeCardOS_200T, генерирует bitstreams и обновляет соответствующим образом Factory_TimeCardOS_200T.bin
- Скрипт */[YOUR_PATH]/Implementation/Xilinx/TimeCard_200T/CreateBinariesGolden.tcl* запускает синтез/реализацию проекта Golden_TimeCardOS_200T, генерирует bitstreams и обновляет соответствующим образом Factory_TimeCardOS_200T.bin
- Скрипт */[YOUR_PATH]/Implementation/Xilinx/TimeCard_200T/CreateBinariesAll.tcl* запускает синтез/реализацию обоих проектов, генерирует соответствующие bitstreams и обновляет соответствующим образом Factory_TimeCardOS.bin. Он также создает файл TimeCardOS_200T_Gotham.bin, который основан на TimeCardOS_200T.bin и имеет дополнительный 16-байтовый заголовок с ID PCIe. 

Бинарные файлы копируются в папку */[YOUR_PATH]/Implementation/Xilinx/TimeCard_200T/Binaries/*. 
...
*/[YOUR_PATH]/Implementation/Xilinx/TimeCard_200T/Binaries/YYYY_MM_DD hh_mm_ss/*, где YYYY: Год, MM: Месяц, DD: День, hh: Час, mm: Минута, ss: Секунда
...
Проект реализован на FPGA [Artix-7 XC7A200T-FBG484-2](https://docs.xilinx.com/v/u/en-US/ug475_7Series_Pkg_Pinout).
...
- Первый образ — это Golden/Fallback образ *Golden_TimeCardOS_200T.bin*. Он содержит только ограниченную функциональность, которая обеспечивает доступ к SPI Flash. 
- Второй образ — это последняя версия обычного образа *TimeCardOS_200T.bin*. Он используется для нормальной работы и является тем, который заменяется при обновлении в полевых условиях.

Конфигурация FPGA всегда начинается с Addr0, где находится Golden образ. Golden образ имеет начальный адрес образа обновления TimeCardOS_200T. 
...
Образ *Factory_TimeCardOS_200T.bin* содержит два bitstreams и должен использоваться для программирования SPI flash в первый раз, например, во время производства. 
...
Образ *TimeCardOS_200T.bin* — это образ обновления/обычный образ и должен использоваться для обновления в полевых условиях через SPI.
Для обновления этот bitstream должен быть размещен по адресу 0x00800000 в SPI flash.
